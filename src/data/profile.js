...
export const projects = [
  ...
  },
  {
    ...
    period: '2026',
    summary:
      '设计并实现前后端分离的实时频道聊天系统，前端基于 Vue 3 + Element Plus 构建响应式聊天界面，后端基于Spring Boot 3提供WebSocket通信，项目成功部署在阿里云服务器。实现了用户注册登录、密码加密存储、头像上传、昵称修改、密码修改等用户体系功能；实现频道消息实时收发，基于 WebSocket 推送文本、图片、文件、语音等多类型消息，并将历史消息持久化到 MySQL；实现 WebRTC 频道实时语音功能；接入 DeepSeek-V4-Bot，支持通过 @bot 在频道内触发 AI 对话；优化移动端聊天体验，加入深色/浅色主题、消息时间显示、附件暂存发送和图片粘贴上传。',
    stack: ['SpringBoot', 'Vue','MyBatis-Plus' ,'MySQL','WebSocket','WebRTC'],
    points: [
